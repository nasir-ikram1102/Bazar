...
function changeIconButtonSearch() {
  if ($(window).width() <= 480) {
    $('.search-box button').html('<i  (click)="typeaheadOnSelect()" class="fa fa-search"></i>');
  } else {
    $('.search-box button').html('<i  (click)="typeaheadOnSelect()" class="fa fa-search"></i>');
  }
}
...
function myFunction() {
  var x = document.getElementById("menucustom");
  if ($(window).width() <= 980) {
...
function zoomIn(event) {
  var fireelement = event.target.id;
  var image = $("#" + fireelement).attr('src');
  $("#overlay").css('background-image', 'url(' + image + ')');
...
$('.payment-tabs-list li').click(function () {
  var tabToOpen = $(this).attr('data-toggle');
  if (!$(this).hasClass('active')) {
    $(this).siblings().removeClass('active');
    $(this).toggleClass('active');
    tabToOpen = $(this).attr('data-toggle');
    $('#' + tabToOpen).siblings().removeClass('active');
