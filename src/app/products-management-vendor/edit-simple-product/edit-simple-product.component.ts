...
@Component({
  ...
  styleUrls: ['./edit-simple-product.component.css']
})
/** edit-simple-product component*/
export class EditSimpleProductComponent {
  ...
  constructor(private _avRoute: ActivatedRoute, private readonly brandsService: BrandService, private modalService: BsModalService, private readonly customerService: CustomerService, public toastr: ToastsManager, vcr: ViewContainerRef,
    private http: HttpClient, private readonly categoryService: CategoryService, private readonly productService: ProductService,
  ...
  getProductById(id: string) {
    this.productService.GetMasterProduct(id).subscribe(data => {
      this.product = data;
      if (this.product.metaDiscription == "" || this.product.metaDiscription == null) {
        this.product.metaDiscription = "";
      }
      if (this.product.metaKeyWords == "" || this.product.metaKeyWords == null) {
        this.product.metaKeyWords = "";
      }
      if (this.product.metaTitle == "" || this.product.metaTitle == null) {
        this.product.metaTitle = "";
      }
    ...
    if (this.files.length > 0) {
      for (var i = 0; i < this.files.length; i++) {
        ...
        if (i + 1 == this.files.length) {
          ...
          this.productService.UpdateSimpleProduct(this.products).subscribe(data => {
